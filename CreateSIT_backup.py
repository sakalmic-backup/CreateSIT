...
def main():
    ...
    # Use the custom CA bundle file (replace with your actual CA certificate path)
    session.verify = r"C:\Users\MSakala\PycharmProjects\SAFe-System_Integration\intranet.frequentis.frq.crt"

    # Create the Jira object using the custom session
    jira = Jira(
        url="https://jiraclone.frequentis.frq",  # Jira URL
        session=session  # Pass the Kerberos-authenticated session
    )
